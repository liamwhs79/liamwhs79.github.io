...
/* Revision 43.6.1 mobile rendering hotfix.
   Keep the animated wallpaper in its own back plane and prevent Android/Samsung
   backdrop-filter compositing tiles from painting over the interface. */
(function(){
 if(document.getElementById('berry4361MobileHotfix')) return;
 const css=String.raw`
html,body{max-width:100%!important;overflow-x:hidden!important}
#app{position:relative!important;isolation:isolate!important;max-width:100%!important;overflow-x:hidden!important}
#themeWallpaper.theme-wallpaper{z-index:-1!important;contain:paint!important}
#app>main{position:relative!important;z-index:1!important;width:100%!important;max-width:100%!important;min-width:0!important;overflow-x:hidden!important}
#app>.topbar{z-index:30!important}
#littleSpaceDock{z-index:120!important}
.page,.page.active,.panel,.card,.home-dashboard,.home-checkin-card,.home-together-card,.store-shell,.profile-panel,.gamepane,.polished-game,.together-pane{max-width:100%!important;min-width:0!important;box-sizing:border-box!important}
.page{overflow-x:hidden!important}
.page.active.talks-page{left:0!important;right:0!important;width:100%!important;max-width:100%!important;min-width:0!important}
.talks-static-shell,.talks-static-head,.talks-thread,.talks-composer,.talks-partner-row,.talks-checkin-grid,.composer-v3{width:100%!important;max-width:100%!important;min-width:0!important;box-sizing:border-box!important}
.talks-thread{position:relative!important;z-index:1!important}
.talks-composer{position:relative!important;z-index:2!important}
.talks-composer .composer-v3 input{width:auto!important;flex:1 1 auto!important;min-width:0!important}
#prizeGarden.page.active{left:0!important;right:0!important;width:100%!important;max-width:100%!important}
.bottomnav{max-width:100vw!important}

/* Mobile Chromium can tile backdrop-filter incorrectly over a transformed fixed wallpaper.
   The cards already have opaque/translucent fills, so removing only backdrop blur keeps the
   visual style while eliminating the giant rectangular paint artefacts. */
@media(max-width:820px){
  #app *,#app *::before,#app *::after,.bottomnav{
    -webkit-backdrop-filter:none!important;
    backdrop-filter:none!important;
  }
  .bh-theme-photo,.bh-theme-depth,.bh-theme-light,.bh-theme-specks,.bh-theme-haze{
    will-change:auto!important;
  }
  .bh-theme-photo{inset:0!important;transform:none!important;animation:bh4361WallpaperBreath 24s ease-in-out infinite alternate!important}
  .bh-theme-light,.bh-theme-haze{filter:none!important}
  .fx-water .depth-a,.fx-water .depth-b,.fx-honey .depth-a,.fx-velvet .depth-a,.fx-satin .depth-a,.fx-clouds .depth-a,.fx-stardust .depth-b{filter:none!important}
  .fx-water .depth-a{animation:bh4361Water 18s linear infinite!important}
  .fx-water .depth-b{animation:bh4361Ray 11s ease-in-out infinite alternate!important}
  .fx-honey .depth-a{animation:bh4361Honey 20s linear infinite!important}
  .fx-clouds .depth-a{animation:bh4361Clouds 26s linear infinite!important}
}
@keyframes bh4361WallpaperBreath{0%{background-position:47% 49%;opacity:.98}100%{background-position:53% 51%;opacity:1}}
@keyframes bh4361Water{0%{background-position:0 0}100%{background-position:80px -120px}}
@keyframes bh4361Ray{0%{background-position:0 50%;opacity:.28}100%{background-position:100% 50%;opacity:.72}}
@keyframes bh4361Honey{0%{background-position:0 0}100%{background-position:160px 80px}}
@keyframes bh4361Clouds{0%{background-position:0 0,0 0,0 0}100%{background-position:180px 0,130px 0,220px 0}}
`;
 const s=document.createElement('style');
 s.id='berry4361MobileHotfix';
 s.textContent=css;
